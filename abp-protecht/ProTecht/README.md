...
* Check the `ConnectionStrings` in `appsettings.json` files under the `ProTecht.HttpApi.Host` and `ProTecht.DbMigrator` projects and change it if you need.

### Before running the application

* Run `abp install-libs` command on your solution folder to install client-side package dependencies. This step is automatically done when you create a new solution, if you didn't especially disabled it. However, you should run it yourself if you have first cloned this solution from your source control, or added a new client-side package dependency to your solution.
* Run `ProTecht.DbMigrator` to create the initial database. This step is also automatically done when you create a new solution, if you didn't especially disabled it. This should be done in the first run. It is also needed if a new database migration is added to the solution later.
...
```bash
dotnet dev-certs https -v -ep openiddict.pfx -p e5ee8a13-818d-48e7-93a3-fd696b597a8d
```

> `e5ee8a13-818d-48e7-93a3-fd696b597a8d` is the password of the certificate, you can change it to any password you want.
...
* `ProTecht.DbMigrator`: A console application which applies the migrations and also seeds the initial data. It is useful on development as well as on production environment.
* `ProTecht.HttpApi.Host`: ASP.NET Core API application that is used to expose the APIs to the clients.
