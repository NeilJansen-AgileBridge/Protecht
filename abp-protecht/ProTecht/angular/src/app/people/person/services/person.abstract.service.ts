...
import type { GetPeopleInput, PersonDto } from '../../../proxy/people/models';
import { PersonService } from '../../../proxy/people/person.service';

export abstract class AbstractPersonViewService {
  protected readonly proxyService = inject(PersonService);
  ...
  data: PagedResultDto<PersonDto> = {
    items: [],
    totalCount: 0,
  };

  selectionType = SelectionType;
  selected = signal<PersonDto[]>([]);
  allSelected = signal(false);
  selectedCount = computed(() => this.selected().length);

  filters = {} as GetPeopleInput;
  ...
  hookToQuery() {
    ...
    const setData = (list: PagedResultDto<PersonDto>) => {
      this.data = list;
  ...
  clearFilters() {
    this.filters = {} as GetPeopleInput;
    this.list.get();
  }
  ...
  exportToExcel() {
    ...
    this.proxyService
      ...
      .subscribe(result => {
        this.abpWindowService.downloadBlob(result, 'Person.xlsx');
      });
  }
}
