...
} from '../../../proxy/quotes/models';
import { QuoteService } from '../../../proxy/quotes/quote.service';

export abstract class AbstractQuoteViewService {
  protected readonly proxyService = inject(QuoteService);
  ...
  data: PagedResultDto<QuoteWithNavigationPropertiesDto> = {
    items: [],
    totalCount: 0,
  };

  selectionType = SelectionType;
  selected = signal<QuoteWithNavigationPropertiesDto[]>([]);
  allSelected = signal(false);
  selectedCount = computed(() => this.selected().length);

  filters = {} as GetQuotesInput;
  ...
  protected bulkDeleteRequest() {
    const ids = this.selected().map(({ quote: { id } }) => id);
  ...
  delete(record: QuoteWithNavigationPropertiesDto) {
    this.confirmationService
      ...
      .pipe(
        ...
        switchMap(() => this.proxyService.delete(record.quote.id)),
      )
  ...
  hookToQuery() {
    ...
    const setData = (list: PagedResultDto<QuoteWithNavigationPropertiesDto>) => {
      this.data = list;
  ...
  clearFilters() {
    this.filters = {} as GetQuotesInput;
    this.list.get();
  }
  ...
  exportToExcel() {
    ...
    this.proxyService
      ...
      .subscribe(result => {
        this.abpWindowService.downloadBlob(result, 'Quote.xlsx');
      });
  }
}
